...
bool SATSolver::solve(const Formula& formula) {
    ...
    if (DPLL(formula, assignment)) {
        // Always print SATISFIABLE and the assignment, regardless of logging level
        std::cout << "SATISFIABLE\n";
        std::cout << "Assignment: ";
        printAssignment(assignment, -1);  // Use -1 to bypass Logger verbosity control
        return true;
    } else {
        // Always print UNSATISFIABLE, regardless of logging level
...
bool SATSolver::DPLL(Formula formula, Assignment& assignment) {
    ...
    // Stopping conditions
    if (formula.empty()) {
        Logger::log(1, "Formula satisfied!");
        return true;
    }
    if (hasEmptyClause(formula)) {
        Logger::log(1, "Encountered an empty clause. Backtracking...");
